...
const tempDir = await mkdtemp(path.join(os.tmpdir(), 'axm-global-state-proof-017-'));
...
const checkpointHead = 'proof-017:mutation-root';
...
try {
  ...
  // Full receipt #6 is gone from the retained suffix, but its compact proposal
  // fingerprint + sequence + accepted head are enough to rebuild the exact
  // original receipt when the exact proposal is retried.
  ...
  // A duplicate still present in the retained suffix follows the ordinary live
  // authority duplicate path.
  ...
  // New accepted history continues from revision 40 with no special sequence lane.
  ...
  // Old clients cannot silently reconstruct from a suffix that starts after the
  // new checkpoint. A separate compatible product/replay checkpoint is required.
  ...
  // Process restart: only the compacted document plus trusted compaction checkpoint
  // are used to recover both old idempotence evidence and the live suffix.
  ...
  // Compact evidence is checksum-bound to the trusted compaction checkpoint.
  ...
  console.log('AXM Global State proof 017 compacted idempotence history: PASS');
  console.log({
