...
const tempDir = await mkdtemp(path.join(os.tmpdir(), 'axm-global-state-proof-019-'));
const compactedPath = path.join(tempDir, 'compacted-history.json');
const mutationRoot = 'proof-019:mutation-root';
...
try {
  ...
  console.log('AXM Global State proof 019 replay checkpoint across compaction floor: PASS');
  console.log({
